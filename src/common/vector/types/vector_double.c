...
vector_double* init_double(size_t size) {
  ...
  vector_double* new_vec = calloc(1, sizeof(vector_double));
  if (!new_vec) return NULL;
  new_vec->size = len;
  new_vec->length = 0;
  new_vec->data = calloc(len, sizeof(double));
...
vector_double* init_data_double(const double* data, size_t size) {
  ...
  refresh_last_first_double(new_vec);
  return new_vec;
}
...
double* set_data_double(vector_double* vec, const double* data, size_t size) {
  ...
  refresh_last_first_double(vec);
  return vec->data;
}
double pop_double(vector_double* vec) {
  if (!vec) return (double)0;
  if (!vec->length) return (double)0;
  double c = *vec->last;
  vec->data = realloc(vec->data, --vec->length * sizeof(double));
  refresh_last_first_double(vec);
  return c;
}
...
size_t push_data_double(vector_double* vec, const double* data, size_t size) {
  ...
  vec->data = realloc(vec->data, n_len * sizeof(double));
...
void free_v_double(vector_double* vec, void (*free_data)(double)) {
  if (free_data) foreach_v_double(vec, free_data);
  free(vec->data);
  free(vec);
}